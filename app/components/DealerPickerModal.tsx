      ...
      >
        ...
        <div className="flex items-start justify-between gap-3 p-5 sm:p-6" style={{ borderBottom: `1px solid ${border}` }}>
          <div className="min-w-0">
            <h3 className="text-lg font-black" style={{ color: textPrimary }}>{pickText(lang, "Bayi Bul & Satın Al", "Find a Dealer & Buy")}</h3>
            <p className="text-xs mt-0.5 truncate" style={{ color: textMuted }}>
              {productName
                ? `${productName} — ${pickText(lang, "yetkili bayilerimizden alın", "buy from our authorized dealers")}`
                : (pickText(lang, "Yetkili bayilerimizden alın", "Buy from our authorized dealers"))}
            </p>
          </div>
          <button
            onClick={onClose}
          ...
          <div className="px-5 sm:px-6 pt-4 flex-shrink-0">
            <div className="flex items-center gap-2 rounded-xl px-3 py-2.5" style={{ background: cardBg, border: `1px solid ${border}` }}>
              ...
              <input
                ...
                placeholder={pickText(lang, "Şehir veya firma ara…", "Search city or company…")}
                aria-label={pickText(lang, "Şehir veya firma ara", "Search city or company")}
              ...
              {query && (
                <button
                  type="button"
                  onClick={() => setQuery("")}
                  aria-label={pickText(lang, "Aramayı temizle", "Clear search")}
        ...
        {cities.length > 0 && (
          <div className="px-5 sm:px-6 pt-3 pb-1 flex flex-wrap gap-2 flex-shrink-0">
            <button onClick={() => setCity("all")} className="text-xs font-semibold px-3 py-1.5 rounded-full transition-all" style={chipStyle(city === "all")}>
              {pickText(lang, "Tümü", "All")} ({totalDealers})
            </button>
            {cities.map((c) => (
        ...
        <div className="flex-1 min-h-0 overflow-y-auto px-5 sm:px-6 py-4 space-y-5">
          {loading && <p className="text-sm text-center py-10" style={{ color: textMuted }}>{pickText(lang, "Yükleniyor…", "Loading…")}</p>}
          {!loading && visibleCities.length === 0 && <p className="text-sm text-center py-10" style={{ color: textMuted }}>{q ? (pickText(lang, "Aramanıza uygun bayi bulunamadı.", "No dealers match your search.")) : (pickText(lang, "Bayi bulunamadı.", "No dealers found."))}</p>}
                  ...
                  <div key={i} className="rounded-2xl p-3.5 sm:p-4" style={{ background: cardBg, border: `1px solid ${border}` }}>
                    ...
                    <div className="flex flex-wrap items-center gap-2 mt-3">
                      {dl.website && (
                        ...
                        >
                          <RiGlobalLine size={14} /> {pickText(lang, "Web Sitesi", "Website")} <RiArrowRightLine size={12} />
                        </a>
                      )}
                      {dl.phone && (
                        <a
                        ...
                        >
                          <RiPhoneLine size={14} /> {pickText(lang, "Ara", "Call")}
                        </a>
                      )}
                      {dl.whatsapp && (
                        <a
