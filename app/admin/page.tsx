                      ...
                      <div className="pt-4 mt-2 border-t border-white/8 space-y-3">
                        <div>
                          ...
                          <p className="text-[11px] text-white/30 leading-snug mt-0.5">
                            Her platform için son paylaşımınızın görselini, açıklamasını ve linkini girin. Anasayfada yorumların yanındaki sosyal medya kartlarında gösterilir. Boş bırakırsanız &quot;Bizi takip et&quot; çağrısı görünür.
                          </p>
                        </div>
                        ...
                        {(["linkedin", "instagram", "youtube"] as const).map((platform) => {
                          ...
                          const setField = (field: keyof SocialPost, value: string) => {
                            ...
                            const updated: SocialPost = { ...cur, [field]: value };
                            const isEmpty = !updated.image && !updated.caption && !updated.link;
                            let next: SocialPost[];
                            if (isEmpty) {
                              next = posts.filter((p) => p.platform !== platform);
                            } else {
                            ...
                            updateContent(["social", "recentPosts"], next);
                          };

                          return (
                            <div key={platform} className="rounded-xl p-3 space-y-2.5"
                              style={{ background: "rgba(255,255,255,0.03)", border: `1px solid ${accent}28` }}>
                              <div className="flex items-center gap-2">
                                <span className="w-2 h-2 rounded-full" style={{ background: accent }} />
                                <span className="text-xs font-bold" style={{ color: "#fff" }}>{label} Son Paylaşımı</span>
                              </div>
                              ...
                              <div className="flex items-center gap-2">
                                {post?.image ? (
                                  ...
                                  <div className="w-14 h-14 rounded-lg flex items-center justify-center flex-shrink-0"
                                    style={{ background: "rgba(255,255,255,0.04)", border: "1px dashed rgba(255,255,255,0.15)", color: "rgba(255,255,255,0.30)" }}>
                                    <span className="text-[10px]">Görsel</span>
                                  </div>
                                )}
                                <label className="flex-1 cursor-pointer inline-flex items-center justify-center gap-1.5 text-xs font-semibold rounded-lg transition-colors"
                                  style={{ background: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.12)", color: "rgba(255,255,255,0.75)", padding: "8px 10px" }}>
                                  ...
                                  <input type="file" accept="image/*" className="hidden"
                                    onChange={async (e) => {
                                      ...
                                      if (!file) return;
                                      try {
                                        const { url } = await uploadImage(file, `social-${platform}`);
                                        setField("image", url);
                                        showToast("ok", "Görsel yüklendi.");
                                      } catch (err) {
                                        showToast("err", `Yükleme başarısız: ${(err as Error).message}`);
                                ...
                                {post?.image && (
                                  <button onClick={() => setField("image", "")} className="text-xs font-semibold rounded-lg"
                                    style={{ background: "rgba(239,68,68,0.10)", border: "1px solid rgba(239,68,68,0.30)", color: "#FCA5A5", padding: "8px 10px" }}
                              ...
                              <textarea
                                ...
                                placeholder="Paylaşım açıklaması (1-2 cümle)"
                                rows={2}
                                className="w-full bg-white/5 border border-white/8 rounded-xl px-3 py-2 text-white text-sm focus:outline-none focus:border-white/22 resize-y"
                              />
                              ...
                              <div className="grid grid-cols-3 gap-2">
                                <input
                                  ...
                                  placeholder={`Paylaşım URL'si (https://...)`}
                                  className="col-span-2 bg-white/5 border border-white/8 rounded-xl px-3 py-2 text-white text-xs focus:outline-none focus:border-white/22 font-mono"
                                />
                                <input
                                  value={post?.date ?? ""}
                                  onChange={(e) => setField("date", e.target.value)}
                                  placeholder="Tarih (ör: 2 hafta önce)"
                                  className="bg-white/5 border border-white/8 rounded-xl px-3 py-2 text-white text-xs focus:outline-none focus:border-white/22"
                                />
                              </div>
                            </div>
                          );
