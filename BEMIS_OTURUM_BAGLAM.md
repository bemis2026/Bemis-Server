...
> **YAPILDI:** `CONTACT_TO_EMAIL` artık **virgülle ayrılmış çoklu adres** kabul ediyor (`lib/email.ts` → `alicilar()`); değer **`satis@bemis.com.tr,info@bemisevcharge.com`** → hiçbir başvuru kaçmaz. `REPLY_TO_EMAIL` = `satis@bemis.com.tr`; ayrıca kodda **verilmemişse BİRİNCİL alıcıdan türetilir** → "başvuru bir kutuya, cevap başka kutuya" ayrışması bir daha oluşamaz.
> **⚠️⚠️ VERCEL ENV TUZAĞI (bu turda 3 kez yanılttı):** `vercel env add` üretimde değeri **VARSAYILAN OLARAK "sensitive"** kaydeder → sonradan `vercel env pull` ile **BOŞ** görünür, değer aslında yazılmıştır. Ayrıca `printf|` ve `< dosya` ile boru **değeri hiç geçirmedi** (boş kaydetti). 📌 **DOĞRU KULLANIM: `vercel env add <AD> production --value "<deger>" --no-sensitive --yes`** — `--no-sensitive` olmadan bir daha okuyup doğrulayamazsın.
> 🔒🔴 **BAYİ E-POSTALARI/TELEFONLARI DIŞARIDAN İNDİRİLEBİLİYOR — HAZIRLIK BİTTİ, TEK TIK KULLANICIDA (2026-08-05):**
> **ÖLÇÜM (parola girmeden, düz `curl`):** `pub-*.r2.dev/bins/dealers.json` → **200**, içinde **29 e-posta (6'sı kişisel gmail/hotmail) · 33 telefon · 10 WhatsApp**. ⚠️ Bu e-postalar **siteye BİLEREK konulmamıştı** ("çoğu kişisel Gmail", 2026-08-02 notu) ama dosya doğrudan indirilebildiği için fiilen açıktalar. `bins/products.json` (1,4 MB) ve `bins/content.json` de aynı şekilde açık.
> **📌 KULLANICI SORUSU + CEVABI:** "admindeyse zaten şifreye gerek yok ki" → **admin paneli EKRANI korur, DOSYAYI değil**; dosyanın kendi genel adresi var ve panelden hiç geçmiyor. (Benzetme: ofis kapısı kilitli ama dolap otoparkta.)
> **KİLİT ÖLÇÜM — genel erişim NEYE gerekliymiş:** `/api/documents` **6** referans (PDF), `/api/products` **0**, `/api/content` **0**, `/api/dealers` **0**. Yani ürün/içerik/bayi verisinin dışa açıklığa **HİÇ** ihtiyacı yok; site onları sunucu tarafında okuyor. Tek bağımlılık, döküman vekilinin dosyayı **genel adresten `fetch` ile** çekmesiydi.
> **YAPILDI:** `lib/store.ts`'e **`readObject(key)`** eklendi; `app/api/documents/file` artık `R2_PUBLIC_URL` ile başlayan URL'leri **S3 API + kimlik** ile okuyor (Cloudinary vb. eskisi gibi HTTP). **Public erişim AÇIKKEN de çalışır** → önce bu yayınlandı, sonra kapatılabilir. **CANLI DOĞRULANDI: 6 R2 dökümanı 200/application/pdf (4,7 MB · 5,3 MB · 5,0 MB… boyutlar tam), 2 Cloudinary dökümanı 200, `dl=1` indirmede dosya adı + Content-Length doğru.**
> **⏳ KULLANICIDA TEK ADIM: Cloudflare → R2 → kova → Settings → Public access (r2.dev) → KAPAT.** Kapanınca `bins/*.json` dışarıdan indirilemez olur; site ve admin paneli etkilenmez. **Kapandıktan SONRA** mesaj arşivi aynı kovaya güvenle açılabilir (şifrelemeye gerek kalmaz).
> **🔴 AÇIK KALAN — MESAJ ARŞİVİ (kullanıcı "gerçekten geri aç" dedi, KULLANICI ADIMI GEREKİYOR):** `writeBin("messages")` bilerek devre dışı; admin panelindeki **"Mesajlar" ekranı bu yüzden HER ZAMAN BOŞ** ve başvurunun tek kopyası e-postada. **Sebep ÖLÇÜLDÜ, varsayım değil:** `pub-*.r2.dev/bins/products.json` ve `bins/content.json` **200 + tam içerik** dönüyor → kova gerçekten herkese açık, oraya PII yazmak isim/telefon/mesajı ifşa ederdi. **Ayrı ÖZEL kova denendi → `AccessDenied`**: R2 anahtarı yalnız mevcut kovaya yetkili, yeni kova açamıyor. **Kullanıcı Cloudflare'den özel bir kova açıp `R2_PRIVATE_BUCKET` env'ini vermeli** (alternatif: mevcut kovaya AES-256-GCM ile ŞİFRELİ yazmak — kod tarafında yapılabilir, kullanıcı onayı bekliyor).

> 🧹🧮 **PRİZ TUTUCU EMEKLİ + AD UYUMU + HESAPLAYICI RÖTUŞLARI (2026-08-05, ürün 151 → 150):** Kullanıcı seçmeli karar verdi.
> **(1) `BAK-8200-0000` "Priz Tutucu" ÜRETİMDEN KALKTI → 12 kaynaktan pozisyonel SİLİNDİ** + eski adresi `next.config.ts`'te **308** ile `AC Soket Tutucu`ya bağlandı. ⚠️ Veriden silmek TEK BAŞINA YETMEZ (adres 404 verirdi) — bu ders 3. kez uygulandı. Betik silmeden ÖNCE 12 kaynağın **hiza doğrulamasını** yapar (uzunluk uyuşmazsa iptal eder).
> **(2) 🔴 YAN BULGU — ad fiyat listesiyle uyumsuzdu:** yeni liste `BAK-8200-0001`'i **"AC Soket Tutucu"** diye yazıyor, sitede hâlâ eski **"Charger Priz Tutucu · Charger Serisi"** duruyordu (DC kardeşi zaten "DC Soket Tutucu"). Ad + alt başlık **12 kaynakta** hizalandı; `productSeo.ts` + `productNamesEn.ts` ("AC Socket Holder") birlikte güncellendi. ⚠️ **`id`/URL DEĞİŞMEDİ** (`charger-priz-tutucu-charger-serisi`) — ad kimlik değil, `id` kimliktir; değiştirmek indekslenmiş adresi 404 yapardı.
> **⚠️⚠️ BU TURDA KENDİ BETİĞİMİN 2 EKSİĞİNİ ÖLÇÜM YAKALADI:** (a) yeniden adlandırma **yalnız R2'nin TR nesnesine** uygulandı, `data/products.json` bayat kaldı (repo = R2 okunamayan build'de FALLBACK → o build eski adı yayınlardı); (b) alt başlık yabancı dillerde ÇEVRİLİ ("Charger-Serie", "Серия Charger") ve **subtitle dile göre merge edilir** → yalnız TR'de temizlemek Almanca ziyaretçiye eski seriyi gösterirdi. 📌 **KURAL: bir alanı 12 kaynakta silerken/değiştirirken, aynı turda o alanın DİL DAVRANIŞINI da sor — `name` TR-kilitli ama `subtitle` DEĞİL.**
> **(3) ✅ İKİ "ÇÖZÜLEMEYEN PDF KODU" KAPANDI (açık iş DEĞİLmiş):** `BD2M-2134-1231-X` ve `BKT-0102-2105` ürün satırında değil **fotoğraf-altı etiket satırında** geçiyor. Birincisi gerçek ürünün (`BD2M2134-1231-X`, tiresiz) etiketi — listede var, fiyatı sitedekiyle aynı (135 €). İkincisi o sayfada ürün satırı olmayan, büyük olasılıkla **`BKT-0102-2205` yazım hatası**. 📌 **Ders: PDF'ten kod çıkarırken ürün satırı ile görsel-altı etiket satırını AYIR** (etiket satırı tek başına kod dizisidir, fiyat taşımaz).
> **(4) HESAPLAYICI — kalan 4 tasarım rötuşu (canlı ölçümle doğrulandı):** başlık çevresindeki **ÜÇ etiket ikiye indi** (pil animasyonu "HESAPLAYICI" rozetinin içine alındı, ayrı "Şarj Simülasyonu" satırı kaldırıldı) · **gösterge büyütüldü** (viewBox 200×215 kare 174×174 kutuya sığdırılıyordu → oran korunarak 196×211 / sm 224×241; mobil 335px kartta taşma yok) · **"Verimlilik" sonuç kutusundan ÇIKARILDI** (3→2 kutu) ve maliyet kutusuna taşındı ("Şebekeden çekilen · Verimlilik %90") — orada durması anlamlı, çünkü faturalanan kWh'in "Enerji" kutusundan neden büyük olduğunu tam o satır açıklıyor; verimlilik hesabın SONUCU değil GİRDİSİ · **tarife kartı ↔ kaydıraç ikiliği çözüldü**: seçili tutar hiçbir yerde görünmüyordu, başlığın sağına canlı rozet eklendi (kart tıklaması 4,50→3,80 ve kaydıraç →7,30 ile takip ettiği ölçüldü).
> **(5) HESAPLAYICI ARAÇ VERİSİ 85 → 88:** Hyundai **Inster Standard** (39 kWh · 11 kW AC · 73 kW DC · 132 Wh/km) + **Inster Long Range** (46 · 11 · 85 · 133) + Citroën **ë-C3** (43,8 · **7,4** kW AC · 97 kW DC · 151 Wh/km) — üçü de **EV Database'den tek tek doğrulandı**, önceki turda kaynak bulunamadığı için eklenmemişti. ⚠️ ë-C3'te standart dahili şarj ünitesi 7,4 kW (11 kW opsiyonel) → kural gereği STANDART değer yazıldı; UI'da "Araç maks: 7.4 kW" olarak göründüğü doğrulandı.
> **(6) 🔴🔴 KENDİ DEĞİŞİKLİĞİMİ CANLIDA DOĞRULARKEN 3 KUSUR DAHA ÇIKTI — ikisi eskiden beri canlıydı:**
> **(a) `metaTitle` R2'de duruyordu → `productSeo.ts` yayına HİÇ ÇIKMIYORDU.** `applyProductSeo`'da **R2 değeri kod haritasını EZER** (bu tuzak bu dosyada zaten yazılıydı, yine düşüldü). BAK-8200-0001'in 6 dil yuvasında da meta\* eski adı taşıyordu ("Charger Priz Tutucu — AC Wallbox Serisi"). **Çözüm: R2'deki meta\* BOŞALTILDI** → tek kaynak koda döndü (yeni ürünlerde zaten bilerek boş bırakılıyor, aynı desen). `specs` "Kategori" değeri de eski ürün adıydı → dile göre güncellendi. 📌 **KURAL: `productSeo.ts` düzenledikten sonra o ürünün R2 meta\* alanının BOŞ olduğunu doğrula, yoksa değişiklik sessizce yutulur.**
> **(b) `BAK-8200-0002` "DC Soket Tutucu" 6 DİLDE DE AC wallbox tutucusunun metnini taşıyordu** — eklenirken kardeşinden klonlanıp uyarlanmamış. **(c) `BAK-8000-0001` "V2L/C2L Kablo Çantası"** da kardeşi BAK-8800-0000'in metnini taşıyordu. İkisi **fiyat listesindeki ÖLÇÜLERLE** ayrıldı (22×26×12 cm ↔ 11×20×8 cm). ⚠️ Ölçü/IP/malzeme UYDURULMADI: ölçüler PDF'ten, "4 bilyalı" ürün adından. BAK-8200-0001'in açıklamasına **DOKUNULMADI** ("Charger serisi AC wallbox için" diyor; ürün kapsamı gerçekten genişledi mi bilinmiyor).
> 📌 **YENİ KALICI DENETİM — REPODA: `scripts/check-clone-descriptions.cjs`** (`npm run check:clones`). Aynı açıklamayı paylaşan ama FARKLI aileden ürünleri bulur (klon-kalıntısı avı). İlk çalıştırmada 4 bulgu: 2'si gerçekti, 2'si aynı ailenin varyantı (yanlış pozitif). **Yeni ürün ekledikten sonra çalıştır.** ⚠️ Build zincirine BİLEREK eklenmedi (marka bekçisinin aksine bu bir uyarı denetimi, deploy durdurmamalı).
> **(8) ✅ MID SAYAÇ + AC TUTUCU KAPSAMI — kullanıcı seçmeli karar verdi, uygulandı:**
> **(a) MID: "dahili" yazılsın.** ⚠️ **Ölçüm 4 değil 8 MID modeli buldu** (Plus 2 / Plus 2 GSM / Pro 2 / Pro 2 GSM × kablolu-pano) ve **daha kötüsü: 4 GSM MID modelinin metni KENDİ KENDİSİYLE ÇELİŞİYORDU** — bir cümle "opsiyonel MID sayaç", başka bir cümle "Bu modelde kasaya entegre… MID enerji sayacı bulunur" diyordu (ikincisi Temmuz'daki GSM klonundan geliyor, TR + 5 dilde). GSM'siz 4 MID modelinde ise entegre cümlesi HİÇ yoktu ve **5 yabancı dilde MID'den söz eden tek kelime bile yoktu.** **YAPILAN:** TR'deki "opsiyonel MID" ibaresi 8 modelden çıkarıldı (2 farklı cümle kalıbı) · entegre cümlesi GSM kardeşinden **AYNI KAYNAK + AYNI DİLDEN kopyalandı** (mevcut onaylı çeviri kullanıldı, yeni metin YAZILMADI) · yeni rozet **`midDahili` "Dahili MID Sayaç"** eklendi (`lib/productFeatures.ts`; ikon `RiDashboard3Line` zaten haritalıydı) ve 8 MID modelinde `mid` → `midDahili` oldu. **Sayaçsız 8 kardeş modele DOKUNULMADI** — orada "(Opsiyonel)" doğru (tarayıcıda ikisi de tek tek doğrulandı). Sayılar birebir: 16 kalıp (8×2 TR kaynağı) · 48 cümle (4×12) · 96 rozet (8×12).
> **(b) AC Soket Tutucu artık her Type 2 soketle:** açıklama seriye bağımlılıktan çıkarıldı, **"gövde rengiyle uyumlu"** iddiası da kaldırıldı (jenerik üründe garanti edilemez); üslup DC kardeşiyle hizalandı, 6 dil.
> **(9) ✅ 23 ÜRÜNÜN 5 DİLDE EKSİK OLAN CÜMLESİ ÇEVRİLDİ (kullanıcı "hepsini çevir" dedi) — 230 alan:** 2026-07-13 broşür turunda TR açıklamalara eklenen cümleler **çeviri katmanına HİÇ geçmemiş** → yabancı ziyaretçi **Otomatik Şarj Başlatma · MID sayaç seçeneği · mobil uygulamadan uzaktan aç/kapa-zamanlayıcı-şarj geçmişi · tak-çalıştır kurulum · güç dengeleme · NFC temassız ödeme** özelliklerini HİÇ görmüyordu (16 wallbox + 7 DC).
> **YÖNTEM — tahmin değil HİZALAMA:** 13 metin grubunun **hepsinde** yabancı dizi TR'den TAM 1 cümle eksikti ve eksik olan **5 bilinen cümleden** biriydi → çeviri o cümlenin **TR'deki indeksine** eklendi (öncesi/sonrası zaten hizalı). Güvenlik ağı: ekleme sonrası cümle sayısı TR ile eşit değilse kaynak ATLANIR. Doğrulama: 5 dilde de "TR'den az cümleli ürün" **0**.
> **🔴 BU İŞ SIRASINDA ÇIKAN AYRI KUSUR — BAYAT `code` ALANI (50 alan):** 2026-07-13'teki DC kod göçü (`BEVDC-40-1` → `BEVDCC-4223-0005`) **yalnız TR'ye** uygulanmış; 10 overlay kaynağında ESKİ kod duruyordu. Görünür etkisi YOK (`code` merge'de TR'den gelir) **ama hiza doğrulaması yapan her betiği "HİZASIZ" sanısına düşürüyor** — bu turda bizzat oldu, çeviri betiği 5 DC ürününü atladı. Overlay `code` alanları TR ile eşitlendi. ⚠️ `name` KASTEN eşitlenmedi (overlay'lerde 220 tarihsel çevrilmiş ad var, merge'de zaten yok sayılıyor). 📌 **KURAL: kod/id göçü yaparken overlay'lerdeki kimlik alanlarını da eşitle — yoksa aylar sonra "hizasız" yanlış-alarmı olarak geri gelir.**
> **(7) 🔴 (a)'YI DÜZELTİRKEN KENDİ REGRESYONUMU YARATTIM — canlı ölçüm yakaladı:** meta\* alanlarını **BOŞ STRING** yapmıştım; ama ürün merge'ü `{...trP, ...enP}` → **overlay'deki `""` TR'nin DEĞERİNİ EZER** → BAK-8200-0001'in **de/es/ar/ru/en sayfalarında metaTitle TAMAMEN SİLİNDİ** (ölçüm: TR 150/150 dolu, DE **149/150** — tek boş olan buydu). **DOĞRUSU: anahtarı SİL** → overlay o alana hiç katılmaz, değer TR'den gelir; TR'de de yoksa `applyProductSeo` kod haritasından doldurur. 📌 **KALICI KURAL: overlay'de bir alanı "iptal etmek" için `""` YAZMA — anahtarı `delete` et.** 📌 **Ders (bu turda 2. kez): düzeltmenin KENDİSİNİ de ölç — "TR'de doğru" görmek yetmiyor, dil başına say.**
> **(10) 🖼️ TEK ÇIKIŞLI C2L ADAPTÖR ARTIK GÖRSELLİ (kullanıcı: V2L ile aynı görsel):** `BKT-2202-2401`, `BKT-2102-2001` "Tek Çıkışlı V2L Adaptör"ün görselini devraldı (`image` + `images`). ⚠️ **Yalnız TR kaynakları yazıldı** — `image`/`images` KİMLİK alanı, merge DAİMA TR'den alır (`route.ts`: `image: trP.image`), overlay'e yazmak gereksiz. Betik hedefte zaten görsel varsa EZMEZ. **ⓘ Görselsiz ürün 13 → 12** (artık Merchant/Meta beslemesine girebilir). store cache **v67-c2l-gorsel**.
> **(11) 🖼️ 8 ADET 30 cm NYAFF PANO PRİZİ DE GÖRSELLENDİ (kullanıcı: "8 adet 30cm olan aynı görseller olabilir SADECE"):** her biri mevcut kardeşinden **yalnız alt başlıktaki "· 30 cm NYAFF"** ile ayrılıyor (ad, açıklama, güç, montaj noktası birebir aynı) → `431B←4310`, `432B←4320`, `-A` varyantlarıyla 8 eşleşme. ⓘ Mevcut 8 kardeşin **hepsi zaten aynı görseli** (`slovarwtn9dt80fx55zc.png`) paylaşıyordu, yani "hepsine tek görsel" ile "her biri kardeşininkini alsın" AYNI sonucu veriyor. Betik ön kontrol yapar (kaynak/hedef/görsel var mı + **ad birebir aynı mı**) ve hedefte görsel varsa EZMEZ. **ⓘ Görselsiz ürün 12 → 4.** store cache **v68-pano-gorsel**.
> **(12) 🏷️ `converters` KATEGORİ ADI DÜZELTİLDİ (kullanıcı bildirdi):** üst menüde **"Uzatma & Kombinasyon"** yazıyordu, alt başlığı da "…& **Kombinasyon Kutuları**". **Ama bu kategoride HİÇ kombinasyon YOK** — ölçüldü: 21 ürünün tamamı Cee Norm/standart adaptör + seyyar uzatma kablosu; **Otomatlı IP44/IP66 Kombinasyonlar (6 ürün) `charger-equipment` altında** (kullanıcı da böyle söyledi). Yeni ad: **"Dönüştürücü Adaptörler ve Uzatma Kabloları"**, 6 dilde (13 kayıt).
> **⚠️ KAYNAK TUZAĞI:** menü adı **ÜRÜN verisinden DEĞİL, İÇERİK (content) katmanından** gelir — `Navbar.categoryList` → `useContent().categories` → `content.categories.<id>`. Ürün bin'indeki `converters.name` ("Uzatmalar ve Dönüştürücüler") **AYRI bir alan**, menüyü beslemiyor. Ayrıca `contentLang` merge'ünde kategori overlay'i TR'yi EZER (`{...tr, ...en}`) → **6 dilin HEPSİ yazılmalı**, yalnız TR yetmez. Kod varsayılanları da güncellendi (`ContentContext.tsx` + `Products.tsx`).
> **📌 BU KATEGORİNİN ADI 4 AYRI YERDE YAŞIYOR** (birini değiştirmek ötekini değiştirmez): **(1)** content bin → **menü + anasayfa kartı** ✅ · **(2)** `Footer.tsx` sabit etiketi "Uzatma & Dönüştürücüler" · **(3)** products bin `converters.name` "Uzatmalar ve Dönüştürücüler" → breadcrumb + ürün detay + /products · **(4)** `seo.ts CATEGORY_SEO.converters.title` "EV Şarj Uzatma & Dönüştürücü Kablo" → **kategori H1 + `<title>`**. **2-3-4 BİLEREK DEĞİŞTİRİLMEDİ:** hiçbiri "Kombinasyon" demiyor (yani kullanıcının bildirdiği hata orada yok), 4 numara ise **kasten anahtar-kelime hedefli** (2026-06-14 turunda öyle yazıldı) — değiştirmek sıralamayı etkiler. Kullanıcıya soruldu. ⓘ `app/data/products.ts` ÖLÜ dosya (hiçbir yerden import edilmiyor), dokunulmadı. store cache **v69-kat-adi**.
> **(13) 🖼️ KATEGORİ GÖRSELLERİ YENİ AMİRAL ÜRÜNLERE ALINDI — wallbox + portable (kullanıcı isteği):** menüde wallbox **ESKİ Charger 2** render'ını gösteriyordu (⚠️ ölçüldü: o görsel **hiçbir üründe kullanılmıyordu**, artık kalmış), portable ise **Mini Mobile**'ı. İkisi de yeni amiral görsellere alındı: **yeni Charger 2** (`wk5wmt8…`) ve **Pro Mobile 2** (`enzqsaqq…`). **3 yüzey birden:** üst menü (`Navbar.CATEGORY_MENU_IMAGE`) + anasayfa kategori kartı + kategori sayfası hero'su (son ikisi aynı alandan: `content.categories.<id>.image`).
> **⚠️ TASARIM: ŞEFFAF ÜRÜN PNG'si `object-cover`'a UYMAZ** — cihaz kırpılır, kablo/soket kadraj dışında kalır. Yeni **`lib/categoryVisual.ts` TEK KAYNAK** (`URUN_PNG_KATEGORI`): bu kategoriler `object-contain` + **HER İKİ temada AÇIK zemin** kullanır. ⚠️ **Zemini tema-bağımlı YAPMA:** yeni Charger 2 **KOYU** gövdeli, Pro Mobile 2 **BEYAZ** gövdeli — koyu zeminde biri, saf beyazda öteki kaybolur; nötr açık gri/gradyan ikisini de taşır (aynı ders ürün kartlarında 2026-07-01'de alınmıştı). Kategori sayfasında `whiteHero` bu listeye bağlandı → **portable otomatik olarak tam-bleed'den çerçeveli panele geçti** (`bgHero = … && !whiteHero`). Doğrulandı: hero 344×344 `object-contain` `#f1f3f6`; anasayfa kartları contain + açık gradyan; **`cables` kontrol kategorisi dokunulmamış** (hâlâ sahne fotosu + cover).
> **🔴 `contentLang.ts` GİZLİ AÇIĞI KAPATILDI:** kategori merge'ünün yorumu "TR image'i korur" diyordu ama **kod `{...tr, ...en}` ile overlay'in `image`'ini geçiriyordu** ve **8 kategorinin 8'inde de overlay'de `image` anahtarı VAR**. Değerler o an aynı olduğu için sapma görünmüyordu — ama TR'de görseli değiştirmek yabancı dillerde **ESKİ görseli bırakırdı** (hero kadrajında bu tam olarak yaşanmıştı, 2026-07-29). Artık `image/descriptionImage/modelCount/badge/comingSoon/heroStyle` **TR-KİLİTLİ**; overlay yalnız `name`/`subtitle` ezebilir. Veri de 6 dilde eşitlendi (26 alan). **📌 KURAL: kategori görseli/düzeni değişince artık YALNIZ TR yeter.**
> **ⓘ SEO ETKİSİ YOK (ölçüldü):** kategori sayfasının `og:image`'ı `ogImage(title)` ile ÜRETİLİR, kategori görselini kullanmaz; `collectionPageSchema` de görsel taşımaz. Menü küçük görselinin `alt=""` olması **DOĞRU** (yanında kategori adı yazılı, ekran okuyucuya tekrar okutmamak için) — değiştirilmedi. store cache **v70-kat-gorsel**.
> **(14) 🗺️ /iletisim HARİTASI ÇALIŞMIYORDU — CSP `frame-src` (kullanıcı bildirdi, 2026-08-05):** sayfa konumu `google.com/maps?...&output=embed` ile `<iframe>` içinde gösteriyor ama **CSP `frame-src` listesinde google.com YOKTU** → tarayıcı çerçeveyi engelliyor, harita yerine BOŞ kutu çıkıyordu. ⚠️ **Bu hata 2026-06-28'de zaten teşhis edilmiş ve düzeltilmişti, ama o tur kullanıcı "sadece değerlendir, değişiklik yapma" dediği için GERİ ALINMIŞTI** — yani bilinen bir açık iki ay açıkta kaldı. `frame-src`'e **`https://www.google.com https://maps.google.com`** eklendi (maps.google.com da: Google bazı bölgelerde oraya yönlendiriyor).
> **DOĞRULAMA (varsayımla değil ölçümle):** embed URL'i **301 ile** `google.com/maps/embed`'e gidiyor; **son 200 yanıtında `X-Frame-Options` YOK** (yalnız 301 üzerinde var, o çerçevelemeyi etkilemez) ve gövdede harita geliyor (`maps.googleapis`, "Bursa" ×6) → **Google gömmeye izin veriyor, tek engel bizim CSP'mizdi.** Yerelde: iframe 1204×360 render, **CSP ihlali 0**, konsolda "Refused to display… X-Frame-Options" hatası YOK. ⚠️ İlk `curl -I` denemem 404 döndürüp beni yanıltacaktı — **Google HEAD isteğine 404 veriyor**; GET + `-L` ile ölç. ⓘ `DealerNetwork` yalnız `href` üretiyor (iframe yok) → etkilenmemişti.
> **(15) 🖱️ /products KATEGORİ ŞERİDİ FAREYLE KAYDIRILAMIYORDU → OK TUŞLARI (kullanıcı bildirdi, 2026-08-05):** `/products` (Tüm Ürünler) üstündeki yapışkan kategori şeridi `overflow-x-auto` + **`scrollbar-hide`** (çubuk gizli) ve **fare tekerleği yatay kaydırmaz** — touchpad kaydırır, fare kaydırmaz → fare kullanan ziyaretçi sağdaki kategorilere **HİÇ ulaşamıyordu** (ölçüm: şerit 1206 px görünür / 1632 px toplam, 426 px erişilemez). Yeni **`app/components/YatayKaydirma.tsx`** (tekrar kullanılabilir): oklar yalnız o yönde içerik VARSA görünür, `hidden sm:flex` (mobilde parmakla kaydırma zaten çalışıyor + oklar çipleri örterdi), kenarda perde gradyanı, `aria-label` 2 dilde, `prefers-reduced-motion`'da animasyonsuz. ⚠️ `cursor-pointer` AÇIKÇA verildi (Tailwind v4 button'a `cursor:default` veriyor).
> **🔴 DOĞRULAMA SIRASINDA ÇIKAN ORTAM GERÇEĞİ — `requestAnimationFrame` BU PANELDE HİÇ ÇALIŞMIYOR:** ölçüldü, **1 saniyede 0 kare**. Bu yüzden hem `scrollBy({behavior:"smooth"})` hem de kendi rAF animasyonum "hiçbir şey yapmıyor" gibi göründü — **kod hatası değil, panel kare üretmiyor** (ekran görüntüsü de aynı sebeple alınamıyor). 📌 **Ders: bu panelde animasyona/kareye bağlı hiçbir şey doğrulanamaz; `scrollLeft` ataması ve `behavior:"auto"` çalışır.** Mantığı doğrulamak için `matchMedia` geçici yamalanıp reduced-motion dalı zorlandı → **tık→426 (sınıra tam kenetleniyor), sağ ok kayboldu/sol ok belirdi; geri→0, tersi** ✅.
> **GÜVENLİK AĞI EKLENDİ:** kare üretmeyen ortamda tuş sessizce hiçbir şey yapmasın diye, süre dolduğunda hâlâ kıpırdamadıysa doğrudan hedefe atlar (ölçüldü: +250 ms bekliyor, +650 ms'de 426). Mobilde ok gizli, sayfa yatay taşması yok.
> **🔴 KENDİ RAPORUM YANLIŞTI — "2 şeritte daha aynı kusur var" DEMİŞTİM, DEĞİLMİŞ:** `scrollbar-hide` geçen dosyaları listeleyip **ok olup olmadığına bakmadan** aynı kusuru varsaymıştım. Kodu açınca: **"Benzer Ürünler"de ZATEN sol/sağ ok var** (daha önce eklenmiş, "Tümünü Gör" butonunun yerine); **`TechnicalSpecs` ise ÖLÜ BİLEŞEN** (hiçbir yerden import edilmiyor, hiç render edilmiyor — `app/data/products.ts` gibi). 📌 **Ders: "aynı kusur şurada da var" demeden önce o dosyayı AÇ.**
> **TAM TARAMA (her `overflow-x-auto` tek tek bakıldı):** gerçekten kusurlu olan **tek yer `/products`'tı**. Öne Çıkanlar/Referans Projeler = marquee + okları var; Blog tabloları, sözlük diyagramı, araç uyumluluk tablosu = gezinme şeridi DEĞİL, **geniş içerik kapsayıcıları** ve kaydırma çubukları **görünür** (fareyle sürüklenebilir, doğru desen).
> **✅ SONRA (kullanıcı "olur" dedi): "Benzer Ürünler" okları da UÇLARDA GİZLENİYOR.** Eskiden hep görünüyorlardı, sona gelince bile duruyorlardı. Ortak mantık `YatayKaydirma.tsx`'ten **hook + yardımcı olarak dışa açıldı** (`useKaydirmaDurumu`, `yumusakKaydir`) → iki şerit aynı davranışı paylaşır ama **GÖRÜNÜMLERİ kendilerinde kalır** (biri kenarda perdeli 32 px ok, öteki içeride accent renkli yüzen 40 px ok). Karusel ayrıca `scrollBy({behavior:"smooth"})`'tan kurtuldu (kare üretmeyen ortamda sessizce ölüydü) + `cursor-pointer` eklendi. ⚠️ Hook bileşenin EN ÜSTÜNDE çağrılıyor (`carouselRef`'in yanında) — karusel JSX'i bir IIFE içinde ve orada koşullu `return null` var, hook oraya konulamaz.
> **⚠️⚠️ BU TURDA BİR TEŞHİSİ YANLIŞ KOYDUM — DÜZELTİLDİ:** karuselde `scrollLeft`'i ELLE değiştirip "sona gidince sağ ok kaybolmuyor" gördüm ve **"donuk dinleyici" hatası** ilan ettim. **YANLIŞTI.** Sonra ölçtüm: **bu panelde `scroll` OLAYI HİÇ TETİKLENMİYOR** — kendi dinleyicim `scrollLeft` 0→600→0 giderken **0 kez** çağrıldı (rAF de 0 kare). Yani oklar bozuk değildi, ben güncellemeyi GÖZLEYEMİYORDUM. 📌 **KURAL: "kod hatası" demeden önce, gözlem yolunun kendisinin çalıştığını doğrula** (olay tetikleniyor mu?) — bu oturumda ölçüm aracını suçlamak yerine sistemi suçlama hatası 3. kez tekrarlandı.
> **DOĞRU TEST YOLU:** bu panelde `scroll` olayı ölü olduğu için ok durumu YALNIZ tuş yolundan güncellenir (`yumusakKaydir` bitince `olc`'yi AÇIKÇA çağırır) → **tuşa tıklayarak** test et, `scrollLeft` atayarak DEĞİL. Öyle test edilince canlıda tam döngü doğrulandı: **x=0 sol ok YOK → ortada İKİSİ de var → x=1492 (son) sağ ok YOK → başa dönünce sol ok YOK.** Her iki şerit de aynı davranıyor.
> ⓘ Efektin bağımlılık dizisi yine de KALDIRILDI (her render'da yeniden bağlanır). Bu bir hata düzeltmesi DEĞİL, savunma amaçlı: `ref.current` gerçekten başka bir DOM düğümü olabilir ve o durumda dinleyici eskisinde kalırdı. Maliyet render başına 1 listener + 1 ResizeObserver — ihmal edilebilir.
> **⛔ KALAN 4 ÜRÜNE GÖRSEL DEVREDİLMEZ (bilerek):** `BEV-3016-4320-1` + `BEV-5032-4320-1` **"Pano Prizi Yeni Tip"** — kullanıcının kendi ifadesiyle bunlar mevcutlarla **birebir aynı özellikte, farkı YALNIZ görselde**; mevcut görseli vermek ürünü yanlış gösterir. `BAK-8000-0001` **Kablo Çantası** (22×26×12) ↔ adaptör çantası (11×20×8) farklı boyut. `BAK-8200-0002` **DC Soket Tutucu** ↔ AC tutucu farklı konnektör. **Bu 4'ü gerçek fotoğraf bekliyor.**
> **⚠️ DEV/BUILD TUZAĞI (bu turda yaşandı):** bu klasörde BAŞKA bir sohbetin dev sunucusu çalışırken `npm run build` çalıştırınca **`.next/dev/types/routes.d.ts` yarım yazıldı** (`nalytics": {}`) → build "Type error: Unexpected keyword or identifier" verdi, `npx tsc --noEmit` ise TEMİZ geçti (farklı include). Ayrıca o dev sunucusunda TÜM rotalar 404 döndü (anasayfa hariç). **Çözüm: tüm `.next`i silme** (diğer oturumu bozar) → **yalnız `rm -rf .next/dev/types`** yeter, Next yeniden üretir. 📌 Build hatası kodda görünmüyorsa önce `.next` altındaki ÜRETİLMİŞ dosyaları şüphelen.
> store cache **v62-tutucu** → **v63-klon-aciklama** → **v64-meta-anahtar**. tsc 0, build 0 (426 sayfa / 150 ürün), tutarlılık denetimi TAM (SEO 150/150, EN ad haritası eksiksiz), klon denetimi 4 → 2 (kalan 2 = yanlış pozitif).
> **⏳ TEK AÇIK İŞ KULLANICIDA: 13 ÜRÜNÜN FOTOĞRAFI** (Priz Tutucu düştüğü için 14 → 13) — sitede görünür ama **Merchant/Meta kataloğuna giremez** (image_link zorunlu). ⓘ Kullanıcı kararı: **BEVDC 180'in 200 kW'a yakın fiyatı (27.500 € ↔ 28.000 €) KASITLI — dokunulmayacak.**

> ✅🛒 **KATALOG BESLEMESİ DOĞRULANDI — 2026-2 TURU KAPANDI (2026-08-04):** `/meta-catalog.xml` uçtan uca ölçüldü: **136 ürün · 13 elenen** (12 yeni görselsiz + eskiden beri görselsiz olan Priz Tutucu) · `g:id` benzersiz, 50 karakteri aşan 0 · görselli 19 yeni ürünün **19'u da feed'de** · görselsiz 12 ürün **beklendiği gibi ELENDİ** (image_link zorunlu) · her item'da görsel+link dolu · **fiyat zinciri birebir**: liste EUR → ×1,20 KDV → ×54,6413 TCMB kuru → feed TRY (3 üründe kontrol, sapma 0). ⚠️ **Doğrulama betiğim iki kez KENDİ hatasıyla "sorun var" dedi** — (a) feed `<g:link>` kullanıyor, ben düz RSS `<link>` aramıştım; (b) düzeltirken `img` alanını yorum satırının içine soktum. 📌 **Ders: "sorun var" çıktısında önce ÖLÇÜM ARACINI şüphelen — canlı sistemi suçlamadan önce ham veriye bak.** Canlı doğrulandı: 15/15 yeni adres 200, 2 yönlendirme 308, SEO başlıkları devrede (BEVDC 180: "DC Hızlı Şarj Ünitesi BEVDC 180 — Çift CCS2"), blogda BEVDC 180 satırı görünüyor, DC kategori metni 6 dilde tazelendi.
> **📧 BAYİ MAİLİ SİTEDE DEĞİL** (kullanıcı isteği): repoya girmedi, `public/`'te yok, 14 dokümanın içinde yok — yalnız `Desktop\Bemis_Bayi_Maili_2026-2_Fiyat_Listesi.md` + `Desktop\Bemis_Fiyat_Listesi_2026-2_Mail_Gorseli.png`.


> 📣📄 **BAYİ MAİLİ + FİYAT LİSTESİ DOKÜMANI + YENİ ÜRÜNLERİN METİNLERİ (2026-08-04):**
> **(1) BAYİ MAİLİ HAZIR** — `Desktop\Bemis_Bayi_Maili_2026-2_Fiyat_Listesi.md` (metin) + `Desktop\Bemis_Fiyat_Listesi_2026-2_Mail_Gorseli.png` (1200×630, PIL ile üretildi; kapak PDF'in 1. sayfasından, marka kilidi de kapaktan kırpıldı — ⚠️ **site logolarında "E-V CHARGE" YOK**, asıl kilit fiyat listesi kapağında). **Metindeki her iddia ölçüldü:** AC düşüşü GERÇEK (git'ten eski↔yeni karşılaştırması: wallbox −%0,6…−5,1 · taşınabilir −%2,6…−9,1 · pano prizi −%7…−11,6, toplam 20 üründe) · 2 yeni DC modeli gerçek · "40'tan 200 kW'a" doğru. ⚠️ **DC ÜNİTELERİNİN FİYATI %0,2–1,1 ARTTI** — mail bunu söylemiyor ama "tüm fiyatlar düştü" de demiyor; **kullanıcı kararı: şeffaflık cümlesi EKLENMEYECEK.** Vade/iskonto/kampanya/geçerlilik yazılmadı (teyit yok).
> **(2) FİYAT LİSTESİ DOKÜMANI DEĞİŞTİ:** eski "Fiyat Listesi 2026" kaydı çıkarıldı, yerine **"Elektrikli Araç Şarj Ürünleri Fiyat Listesi 2026-2"** (R2'ye yüklendi, 4,5 MB, HTTP 200). Kapak PDF'in 1. sayfasından üretildi (480×640, mevcut kapaklarla aynı biçim). ⚠️ Eski doküman sayfası için **308 yönlendirme** eklendi — veriden silmek tek başına yetmez, adres 404 verirdi.
> **(3) YENİ ÜRÜNLERİN METİNLERİ İŞLENDİ (kullanıcı 4 kalemi de seçti):** **(a) DC kategori metni** — SSS'te "BEVDC-80, 120, 160 ve 200 çift soketlidir" deniyordu, **180 eksikti**; açıklamaya da model listesi + direk tipi notu eklendi. **6 dilde elle yazıldı** (MyMemory model adlarını bozuyor), **26 alan**, uyarı 0. **(b) Blog** — iki karşılaştırma tablosuna BEVDC 180 satırı, iki paragrafa direk tipi notu. **(c) SEO metinleri** — `app/lib/productSeo.ts`'e **37 kayıt** (31 yeni + önceki turdan SEO'suz kalan 6 GSM varyantı). Artık **149 ürün / 149 SEO kaydı birebir**, kopya başlık 0, 62 karakteri aşan 0. **(d) katalog beslemesi doğrulaması** deploy sonrasına kaldı.
> **🔴 BU TURDA ÖLÇÜMÜN YAKALADIĞI 3 HATA:** (i) **`pro-mobile-2-11kw-5m` alt başlığı çelişkiliydi** — "Üç Fazlı · 11 - 22 kW · 11 kW" (şablonun güç aralığı kalmış); 12 kaynakta "11 kW · 5 m" yapıldı. (ii) **DC ürünlerinin SEO metni IP65 diyordu, gerçek spec IP54** — 5 kayıtta düzeltildi (Google sonucunda yayınlanan yanlış spec iddiası). (iii) **SEO eklemesi ilk denemede ProductSeo TİP tanımının içine yazıldı** (dosyadaki ilk `};` onunki) → tsc yakaladı, yedekten geri alınıp doğru çapaya (`export const PRODUCT_SEO` sonrası ilk kapanış) taşındı. 📌 **Ders: bir dosyaya kod eklerken "ilk kapanış parantezi" çapası güvenilmez — bildirimden itibaren ara.**
> store cache **v55-dc-metin-seo**. tsc 0, build 0 (424 sayfa). ⏳ **Kullanıcı kararı: bemis.com.tr'deki eski fiyat listesi PDF'i ve B2B portalı ŞİMDİLİK ELE ALINMAYACAK.**
