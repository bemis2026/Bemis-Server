/**
 * TEK SEFERLİK: 19 AC wallbox ürününün BAYAT `meta*` alanlarını veri katmanından SİLER.
 *
 * 🔴 NEDEN (kayıtlı tuzak — `npm run check:seo` 2026-09-15'te YİNE yakaladı):
 *    `applyProductSeo` → `pick(veri, kod)`: veri katmanındaki (R2 / repo yedeği)
 *    `meta*` değeri DOLUYSA `app/lib/productSeo.ts` haritasını **EZER**.
 *    Bu ürünlerin R2 kayıtlarında ESKİ (ev/teknik dilindeki) metinler duruyordu →
 *    koda yazılan yeni ortak-alan metinleri CANLIYA HİÇ ÇIKMADI.
 *    Bekçi: "VERİ KATMANI KOD HARİTASINI EZİYOR: 20 alan · 10 ürün".
 *
 * ⚠️ 10 ürün AYRIŞMIŞTI ama 19'un HEPSİ temizlenir: kalan kayıtlar da kodun ESKİ
 *    kopyası; bugün aynı göründükleri için bekçi susar, yarın kodu düzenleyince
 *    aynı tuzak sessizce geri gelir. Kategoride tek kaynak = kod haritası.
 *
 ...
 *   node scripts/wallbox-meta-temizle.cjs          # KURU ÇALIŞMA
 *   node scripts/wallbox-meta-temizle.cjs --yaz
 *   rm -f .env.sentinel
 */
...
const KIMLIKLER = new Set([
  "charger-2-kablolu", "charger-2-fisli", "charger-2-trifaze-kablolu",
  "charger-plus-2-kablolu", "charger-plus-2-pano",
  "charger-plus-2-gsm-kablolu", "charger-plus-2-gsm-pano",
  "charger-plus-2-gsm-mid-kablolu", "charger-plus-2-gsm-mid-pano",
  "charger-plus-2-mid-sayacli-kablolu", "charger-plus-2-mid-sayacli-pano-prizli",
  "charger-pro-2-kablolu", "charger-pro-2-pano",
  "charger-pro-2-gsm-kablolu", "charger-pro-2-gsm-pano",
  "charger-pro-2-gsm-mid-kablolu", "charger-pro-2-gsm-mid-pano",
  "charger-pro-2-mid-sayacli-kablolu", "charger-pro-2-mid-sayacli-pano-prizli",
]);
...
(async () => {
  ...
  for (const BIN of BINLER) {
    ...
    fs.writeFileSync(path.join("scratchpad", `_${BIN}.wbmetabak.json`), JSON.stringify(veri), "utf-8");
    ...
    if (bulunan.length === 0) { console.log("  bu shard'da wallbox urunu yok → atlandi."); continue; }
